...
namespace ROMUtils
{
    ...
    /// </summary>
    /// <remarks>
    /// The pointer which is returned does not include the upper byte, which is only necessary for
    /// the GBA memory map. The returned int value can be used to index the ROM data.
    /// </remarks>
    /// <param name="data">
    /// The ROM data to read from.
    /// </param>
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (ROMUtils::CurrentFile[address++] == 1)
            {
                while (1)
                {
                    ...
                    {
                        runData = ctrl & 0x7F;
                        for (int j = 0; j < runData; j++) { dst[2 * j] = CurrentFile[address]; }
                        address++;
                    }
                    else
                    {
                        runData = ctrl;
                        for (int j = 0; j < runData; j++) { dst[2 * j] = CurrentFile[address + j]; }
                        address += runData;
                    }
            ...
            else // RLE16
            {
                ...
                {
                    int ctrl =
                        (static_cast<int>(CurrentFile[address]) << 8) | CurrentFile[address + 1];
                    address += 2; // offset + 2
                    if (!ctrl) { break; }
    ...
    /// </summary>
    /// <remarks>
    /// the first and second byte as the layer width and height information will not be generated in
    /// the function you have to add them by yourself when saving compressed data.
    /// </remarks>
                                  ...
                                  unsigned char **OutputCompressedData)
    {
        ...
        unsigned int lowerLength = Lower->GetCompressedLength(),
                     upperLength = Upper->GetCompressedLength();
            ...
            else
            {
                ...
                if (ValidRATS(ROMData + startAddr + freeBytes))
                {
                    ...
                    unsigned short chunkLen =
                        *reinterpret_cast<unsigned short *>(ROMData + startAddr + freeBytes + 4);
    ...
    bool SaveFile(QString filePath)
    {
        ...
        int levelHeaderOffset = WL4Constants::LevelHeaderIndexTable +
                                currentLevel->GetPassage() * 24 + currentLevel->GetStage() * 4;
        ...
        for (int i = 0; i < chunks.size(); ++i) { chunkIDtoIndex[chunks[i].index] = i; }
        ...
        foreach (struct SaveData chunk, chunks)
        {
            if (chunk.old_chunk_addr > WL4Constants::AvailableSpaceBeginningInROM)
            {
                ...
                if (ValidRATS(RATSaddr)) { strncpy((char *) RATSaddr, "STAR_INV", 8); }
                else
                    std::cout << "Invalid RATS identifier for existing chunk, index " << chunk.index
                              << std::endl;
            }
        }
        ...
        foreach (struct SaveData chunk, chunks)
        {
            ...
            // We add 12 to the pointer location because the chunk ptr starts at the chunk's RATS
            // tag
        ...
        foreach (struct SaveData chunk, chunks)
        {
            ...
            if (chunk.size & 0xFFFF0000)
            {
                ...
                QMessageBox::warning(
                    singleton, "RATS chunk too large",
        ...
        { // Prevent goto from crossing initialization of variables here
            ...
            if (file.isOpen()) { file.write((const char *) TempFile, TempLength); }
            else
            {
            ...
            // Set the new internal data pointer for LevelComponents objects, and mark dirty objects
            // as clean
            ...
            for (unsigned int i = 0; i < rooms.size(); ++i)
            {
                struct LevelComponents::__RoomHeader *roomHeader =
                    (struct LevelComponents::__RoomHeader
                         *) (CurrentFile + roomHeaderInROM +
                ...
                for (unsigned int j = 0; j < 3; ++j) { room->SetEntityListDirty(j, false); }
                ...
                // TODO: if more elements in the game are going to be support customizing in the
                // editor, more pointers need to be reset here
            }
        }
