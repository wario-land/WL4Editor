...
namespace ROMUtils
{
    ...
    void GenerateTilesetSaveChunks(int TilesetId, QVector<SaveData> &chunks)
    {
        ...
        struct ROMUtils::SaveData Map16EventTablechunk = {
            static_cast<unsigned int>(tilesetPtr + 28),
            0x600,
        ...
        struct ROMUtils::SaveData FGTile8x8GraphicDataChunk = {
            static_cast<unsigned int>(tilesetPtr),
        ...
        for (int j = 0; j < 0x300; ++j)
        {
            ...
            map16tilePtr[j * 4 + 2] =
                map16data[j]->GetTile8X8(LevelComponents::TileMap16::TILE8_BOTTOMLEFT)->GetValue();
            map16tilePtr[j * 4 + 3] =
                map16data[j]->GetTile8X8(LevelComponents::TileMap16::TILE8_BOTTOMRIGHT)->GetValue();
        }
                  ...
                  const std::function<void(unsigned char *, std::map<int, int>)> &PostProcessingCallback)
    {
        ...
        foreach (struct SaveData chunk, chunks)
        {
            ...
            *reinterpret_cast<unsigned int *>(ptrLoc) =
                static_cast<unsigned int>((indexToChunkPtr[chunk.index] + 12) | 0x8000000);
        }
    ...
    bool SaveLevel(QString filePath)
    {
        ...
        // Save the level
        bool ret =
            SaveFile(std::move(filePath), chunks, nullptr,
                     [levelHeaderPointer, currentLevel, roomHeaderChunk,
                      &roomHeaderInROM](unsigned char *TempFile, std::map<int, int> indexToChunkPtr) {
                         ...
                         for (int i = 0; i < 92; ++i)
                         {
                             if (singletonTilesets[i]->IsNewTileset())
                             {
                                 ...
                                 memcpy(TempFile + i * 32 + WL4Constants::AnimatedTileIdTableCase2,
                                        (unsigned char *) AnimatedTileInfoTable, 32);
