...
private:
    ...
    bool UnsavedChanges = false; // state check bool only be used when user try loading another ROM,
                                 // another Level or close the editor without saving changes
...
public:
    ...
    {
        CurrentLevel->GetRooms()[selectedRoom]->DeleteEntity(EntityIndex);
    }
