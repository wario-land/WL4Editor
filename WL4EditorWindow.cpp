...
/// <summary>
/// Present the user with an "open file" dialog, and perform necessary level loading actions if a
/// ROM in successfully loaded.
/// </summary>
/// <remarks>
...
void WL4EditorWindow::OpenROM()
{
    ...
    if (!UnsavedChangesPrompt(
            tr("There are unsaved changes. Discard changes and load ROM anyway?")))
        return;

    // Select a ROM file to open
    QString qFilePath = QFileDialog::getOpenFileName(this, tr("Open ROM file"), dialogInitialPath,
...
{
    EditModeWidget->SetLayersCheckBoxEnabled(
        0, CurrentLevel->GetRooms()[selectedRoom]->GetLayer(0)->IsEnabled());
    EditModeWidget->SetLayersCheckBoxEnabled(
        1, CurrentLevel->GetRooms()[selectedRoom]->GetLayer(1)->IsEnabled());
    EditModeWidget->SetLayersCheckBoxEnabled(
        2, CurrentLevel->GetRooms()[selectedRoom]->GetLayer(2)->IsEnabled());
    EditModeWidget->SetLayersCheckBoxEnabled(
        3, CurrentLevel->GetRooms()[selectedRoom]->GetLayer(3)->IsEnabled());
    EditModeWidget->SetLayersCheckBoxEnabled(
        7, CurrentLevel->GetRooms()[selectedRoom]->IsLayer0ColorBlendingEnabled());
}
                                      ...
                                      DialogParams::RoomConfigParams *nextroomconfig)
{
    ...
    if (nextroomconfig->CurrentTilesetIndex != currentroomconfig->CurrentTilesetIndex)
    {
        ...
        currentTileset =
            new LevelComponents::Tileset(tilesetPtr, nextroomconfig->CurrentTilesetIndex);
    ...
    {
        currentRoom->GetLayer(2)->CreateNewLayer_type0x10(nextroomconfig->RoomWidth,
                                                          nextroomconfig->RoomHeight);
    }
    ...
    if (!currentroomconfig->Layer0Enable && nextroomconfig->Layer0Enable)
    {
        ...
        {
            currentRoom->GetLayer(0)->CreateNewLayer_type0x10(nextroomconfig->RoomWidth,
                                                              nextroomconfig->RoomHeight);
        }
        else
        {
            ...
            currentLayer0 = new LevelComponents::Layer(nextroomconfig->Layer0DataPtr,
                                                       LevelComponents::LayerTile8x8);
    ...
    if (nextroomconfig->BackgroundLayerEnable)
    {
        ...
        currentLayer3 = new LevelComponents::Layer(nextroomconfig->BackgroundLayerDataPtr,
                                                   LevelComponents::LayerTile8x8);
        ...
        nextroomconfig->RoomHeight != currentroomconfig->RoomHeight)
    {
        ...
        for (uint i = 0; i < doornum; i++)
        {
            if ((doorlist[i]->GetX2() >= nxtRoomWidth) || (doorlist[i]->GetY2() >= nxtRoomHeight))
            {
                ...
                {
                    deleteDoorIdlist[k--] =
                        i + 1; // the id list will be something like: 0 0 0 8 4 2
                }
            }
        }
        for (uint i = 0; i < doornum; i++)
        {
            if (deleteDoorIdlist[i] != 0)
            {
                if (deleteDoorIdlist[i] != vortexdoorId_needResetPos)
                {
                    if (i == doornum - 1 &&
                        doorcount ==
                            1) // don't delete the last door if there is no vortex door in this Room
                    {
                        currentRoom->GetDoor(deleteDoorIdlist[i] - 1)
                            ->SetDoorPlace(
                                qMin(nxtRoomWidth - 1,
                                     currentRoom->GetDoor(deleteDoorIdlist[i] - 1)->GetX1()),
                                qMin(nxtRoomWidth - 1,
                                     currentRoom->GetDoor(deleteDoorIdlist[i] - 1)->GetX2()),
                                qMin(nxtRoomHeight - 1,
                                     currentRoom->GetDoor(deleteDoorIdlist[i] - 1)->GetY1()),
                                qMin(nxtRoomHeight - 1,
                                     currentRoom->GetDoor(deleteDoorIdlist[i] - 1)->GetY2()));
                        break;
                    }
        ...
        for (uint i = 0; i < 3; i++)
        {
            std::vector<struct LevelComponents::EntityRoomAttribute> entitylist =
                currentRoom->GetEntityListData(i);
            size_t entitynum = entitylist.size();
            for (uint j = entitynum; j > 0; j--)
            {
                if ((entitylist[j - 1].XPos > (nxtRoomWidth - 1)) ||
                    (entitylist[j - 1].YPos > (nxtRoomHeight - 1)))
                {
        ...
        k = limitatornum - 1;
        uint *deleteLimitatorIdlist =
            new uint[limitatornum](); // set them all 0, index the limitator from 1
        for (uint i = 0; i < limitatornum; i++)
        {
            int x2_prime = (limitatorlist[i]->ChangeValueOffset == 1)
                               ? (limitatorlist[i]->ChangedValue)
                               : (limitatorlist[i]->x2);
            int y2_prime = (limitatorlist[i]->ChangeValueOffset == 3)
                               ? (limitatorlist[i]->ChangedValue)
                               : (limitatorlist[i]->y2);
            if ((x2_prime >= nxtRoomWidth) || (y2_prime >= nxtRoomHeight))
            {
                deleteLimitatorIdlist[k--] =
                    i + 1; // the id list will be something like: 0 0 0 8 4 2
            }
        }
        for (uint i = 0; i < limitatornum; i++)
        ...
        for (int i = 0; i < 3; ++i)
        {
            if (currentRoom->GetLayer(i)->GetMappingType() == LevelComponents::LayerMap16)
            {
                if (currentroomconfig->LayerData[i] == nullptr)
                {
                    ...
                    size_t datasize1 =
                        2 * currentroomconfig->RoomWidth * currentroomconfig->RoomHeight;
                    ...
                    // reset Layer size
                    currentRoom->GetLayer(i)->ChangeDimensions(nextroomconfig->RoomWidth,
                                                               nextroomconfig->RoomHeight);
    ...
    // reset all the Parameters in Room class. TODO: except new layer data pointers, generate them
    // on saving
    ...
    // reset LayerDataPtr in RoomHeader because Layer::SetDisabled() cannot change the data in
    // RoomHeader
    for (int i = 0; i < 3; ++i)
    {
        if (currentRoom->GetLayer(i)->GetMappingType() == LevelComponents::LayerDisabled)
        {
            currentRoom->SetLayerDataInRoomHeader(
                i, WL4Constants::NormalLayerDefaultPtr); // TODO: need a fix for a Tileset in toxic
                                                         // landfill
        }
    }
...
void WL4EditorWindow::DeleteDoor(int globalDoorIndex)
{
    ...
    CurrentLevel->GetRooms()[CurrentLevel->GetDoors()[globalDoorIndex]->GetRoomID()]->DeleteDoor(
        globalDoorIndex);

    // Disable the destination for all the existing Doors whose DestinationDoor is the Door which is
    // being deleting
...
void WL4EditorWindow::RenderScreenFull()
{
    ...
    QGraphicsScene *scene = CurrentLevel->GetRooms()[selectedRoom]->RenderGraphicsScene(
        ui->graphicsView->scene(), &renderParams);
                                             ...
                                             int LayerID)
{
    ...
    CurrentLevel->GetRooms()[selectedRoom]->RenderGraphicsScene(ui->graphicsView->scene(),
                                                                &renderParams);
}

/// <summary>
/// Override the close window functionality so that a save prompt is offered if there are unsaved
/// changes.
/// </summary>
/// <param name="event">
...
void WL4EditorWindow::on_loadLevelButton_clicked()
{
    // Check for unsaved operations
    if (!UnsavedChangesPrompt(
            tr("There are unsaved changes. Discard changes and load level anyway?")))
        return;
...
bool WL4EditorWindow::UnsavedChangesPrompt(QString str)
{
    if (UnsavedChanges)
    {
        ...
        QPushButton *discardButton =
            savePrompt.addButton(tr("Discard"), QMessageBox::DestructiveRole);
        QPushButton *cancelButton = savePrompt.addButton(tr("Cancel"), QMessageBox::NoRole);
        QPushButton *saveButton = savePrompt.addButton(tr("Save"), QMessageBox::ApplyRole);
        savePrompt.setDefaultButton(cancelButton);
        savePrompt.exec();
...
void WL4EditorWindow::CurrentRoomClearEverything()
{
    ...
    IfDeleteDoors.setText("You just triggered the clear-all shortcut (current room).\nDo you want "
                          "to delete all the doors, too?\n(One door will be kept to render camera "
                          "boxes correctly.\nCamera settings will be unaffected regardless.)");
    QPushButton *CancelClearingButton =
        IfDeleteDoors.addButton(tr("Cancel Clearing"), QMessageBox::RejectRole);
    ...
    if (IfDeleteAllDoors)
    {
        ...
        for (uint i = 0; i < doornum; i++)
        {
            if (deleteDoorIdlist[i] != 0)
            {
                ...
                {
                    if (i == doornum - 1 &&
                        !vortexdoorId_needResetPos) // don't delete the last door if there is no
                                                    // vortex door in this Room
                        break;
                    if (i == doornum - 1 &&
                        vortexdoorId_needResetPos) // delete the last door if there is a vortex door
                                                   // in this Room
                        continue;
                    DeleteDoor(currentRoom->GetDoor(deleteDoorIdlist[i] - 1)->GetGlobalDoorID());
                    // Seems don't need to set Door dirty at least for now
...
void WL4EditorWindow::on_actionLevel_Config_triggered()
{
    ...
    if (acc == QDialog::Accepted)
    {
        ...
        CurrentLevel->SetTimeCountdownCounter(LevelComponents::HardDifficulty,
                                              (unsigned int) dialog.GetHModeTimer());
        CurrentLevel->SetTimeCountdownCounter(LevelComponents::NormalDifficulty,
                                              (unsigned int) dialog.GetNModeTimer());
        CurrentLevel->SetTimeCountdownCounter(LevelComponents::SHardDifficulty,
                                              (unsigned int) dialog.GetSHModeTimer());
        SetUnsavedChanges(true);
    }
}
...
/// </summary>
/// <remarks>
/// Without this override, the edit mode widget and tile 16 selector widget will each take up half
/// the space on the side of the main window if they are both docked.
/// </remarks>
/// <param name="event">
...
/// <summary>
/// Show the user a dialog for configuring the current room. If the user clicks OK, apply selected
/// parameters to the room.
/// </summary>
void WL4EditorWindow::on_actionRoom_Config_triggered()
{
    // Set up parameters for the currently selected room, for the purpose of initializing the
    // dialog's selections
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        operation->lastRoomConfigParams =
            new DialogParams::RoomConfigParams(*_currentRoomConfigParams);
        operation->newRoomConfigParams =
            new DialogParams::RoomConfigParams(dialog.GetConfigParams());
...
void WL4EditorWindow::on_actionNew_Door_triggered()
{
    ...
    LevelComponents::Door *newDoor = new LevelComponents::Door(
        newDoorEntry, (unsigned char) selectedRoom, CurrentLevel->GetDoors().size());
    newDoor->SetEntitySetID(
        (unsigned char) CurrentLevel->GetRooms()[selectedRoom]->GetCurrentEntitySetID());
...
bool WL4EditorWindow::SaveCurrentFileAs()
{
    QString qFilePath = QFileDialog::getSaveFileName(
        this, tr("Save ROM file as"), dialogInitialPath, tr("GBA ROM files (*.gba)"));
...
void WL4EditorWindow::on_actionAbout_triggered()
{
    ...
    if(infoPrompt.clickedButton() == changelogButton)
    {
        // Get the changelog
        const QString
    URI("https://raw.githubusercontent.com/Goldensunboy/WL4Editor/master/LICENSE"); QUrl URL =
    QUrl::fromEncoded(URI.toLocal8Bit()); QNetworkRequest request(URL); QNetworkAccessManager
    manager; QNetworkReply *reply = manager.get(request); QString errorText = reply->errorString();
...
void WL4EditorWindow::on_action_swap_Layer_0_Layer_1_triggered()
{
    ...
    if (CurrentLevel->GetRooms()[selectedRoom]->GetLayer(0)->GetMappingType() !=
        LevelComponents::LayerMap16)
    {
        statusBarLabel->setText("Swaping Layers failed!");
        return;
    }
...
{
    QString qFilePath =
        QFileDialog::getSaveFileName(this, tr("Save current Room graphic to new file"),
