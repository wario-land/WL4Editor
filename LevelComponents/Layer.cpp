...
namespace LevelComponents
{
            ...
            MappingType(mappingType), Enabled(mappingType != LayerDisabled), DataPtr(layerDataPtr)
    {
        ...
        if (mappingType == LayerMap16)
        {
            ...
            // Get the layer data
            LayerData = reinterpret_cast<unsigned short *>(
                ROMUtils::LayerRLEDecompress(layerDataPtr + 2, Width * Height * 2));
        }
        else if (mappingType == LayerTile8x8)
        {
            ...
            // Get the layer data
            LayerData = reinterpret_cast<unsigned short *>(
                ROMUtils::LayerRLEDecompress(layerDataPtr + 1, Width * Height * 2));
    ...
    Layer::Layer(Layer &layer) :
            MappingType(layer.MappingType), Enabled(layer.Enabled), Width(layer.Width),
            Height(layer.Height), LayerPriority(layer.LayerPriority), dirty(layer.dirty),
    ...
    void Layer::DeconstructTiles()
    {
        ...
        // If it is map16 type, then they are just pointer copies and should be deconstructed in
        // ~Tileset() only
    ...
    /// </summary>
    /// <remarks>
    /// The tileset parameter is necessary because tileset information is in the Room object, not
    /// layer.
    /// </remarks>
    /// <param name="tileset">
    ...
    void Layer::ReRenderTile(int X, int Y, unsigned short TileID, Tileset *tileset)
    {
        ...
        else if (MappingType == LayerTile8x8)
        {
            ...
            std::cout << "WARNING: Invalid mapping type ecountered in Layer::ChangeTile"
                      << std::endl;
    }
    ...
    {
        if (!LayerData) return;
        if (MappingType == LayerTile8x8) // If this is mapping type tile8x8, then the tiles are heap
                                         // copies of tileset tiles.
        {
    ...
    unsigned char *Layer::GetCompressedLayerData(unsigned int *dataSize)
    {
        ...
        unsigned int compressedSize =
            ROMUtils::LayerRLECompress(Width * Height, LayerData, &dataBuffer);
