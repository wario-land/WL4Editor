...
namespace LevelComponents
{
    ...
    /// </summary>
    /// <remarks>
    /// Mapping type is a parameter because that information is not contained in the layer data
    /// itself.
    /// </remarks>
    ...
    Tileset::Tileset(int tilesetPtr, int __TilesetID)
    {
        ...
        for (int i = 0; i < 16; ++i)
        {
            ...
            for (int j = 1; j < 16; ++j)
            {
                unsigned short color555 =
                    *(unsigned short *) (ROMUtils::CurrentFile + subPalettePtr + j * 2);
        ...
        {
            /*
             * the reason why not using this is that it will cause problem and worse in some
            situation,
             * so before we know what the first case really used for, we should just use the simple
            one.
             * TODO: find the usage of the first case
            //[0300002E..03000032] are all set to zero at 6B8FA and the arrange just contains all
            the values the table start from 0x3F8C18 have if(ROMUtils::CurrentFile[0x3F8C18 +
            __TilesetID * 16 + v1] & 1) tmpAnimatedTilesHeaderPtr = 0x3F7828 + (int) (8 *
            (*(unsigned short*) (ROMUtils::CurrentFile + __TilesetID * 32 + 2 * v1 + 0x3F91D8)));
            else
                tmpAnimatedTilesHeaderPtr = 0x3F7828 + (int) (8 * (*(unsigned short*)
            (ROMUtils::CurrentFile + __TilesetID * 32 + 2 * v1 + 0x3F8098)));
            */
            tmpAnimatedTilesHeaderPtr =
                0x3F7828 + (int) (8 * (*(unsigned short *) (ROMUtils::CurrentFile +
            ...
            {
                tile8x8data[i + 4 * v1] =
                    new Tile8x8(tmpAnimatedTilesdataPtr + tmpoffset + i * 32, palettes);
            }
        }
        ...
        {
            tile8x8data[tile8x8size - 1 - bgGFXcount + i] =
                new Tile8x8(bgGFXptr + i * 32, palettes);
        }
        ...
        {
            unsigned short *map16tilePtr =
                (unsigned short *) (ROMUtils::CurrentFile + map16ptr + i * 8);
            Tile8x8 *tiles[4];
            for (int j = 0; j < 4; ++j)
            {
        ...
        Map16EventTable =
            (unsigned short *) (ROMUtils::CurrentFile + ROMUtils::PointerFromData(tilesetPtr + 28));
    ...
    QPixmap Tileset::Render(int columns)
    {
        ...
        for (int c = 0; c < columns; ++c)
        {
            for (int i = 0; i < tileCountY; ++i)
            {
                for (int j = 0; j < 8; ++j)
                {
                    map16data[(c * tileCountY + i) * 8 + j]->DrawTile(&pixmap, (c * 8 + j) * 16,
                                                                      i * 16);
                }
            }
        }
