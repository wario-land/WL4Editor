...
{
    /// <summary>
    /// This is a helper funtion that allows you to load a level based on a passage and stage
    /// number. This constructor will chain to the other constructor.
    /// </summary>
    /// <remarks>
    /// Passage numbers:
    ///      0x00: Entry Passage
    ...
    ///           0x01: When Wario moves offscreen vertically, the camera will follow in
    ///           screen-sized increments 0x02: The camera will freely follow Wario 0x03: Camera
    ///           scrolling will be controlled by special parameters
    ...
    Level::Level(enum __passage _passage, enum __stage _stage) : passage(_passage), stage(_stage)
    {
        ...
        memcpy(&LevelHeader, ROMUtils::CurrentFile + levelHeaderPointer,
               sizeof(struct __LevelHeader));

        // Load the door data
        std::vector<int> destinations;
        int doorStartAddress = ROMUtils::PointerFromData(WL4Constants::DoorTable + LevelID * 4);
        struct __DoorEntry *doorPtr =
            (struct __DoorEntry *) (ROMUtils::CurrentFile + doorStartAddress);
        ...
        // Load the level name
        int LevelNameAddress = ROMUtils::PointerFromData(WL4Constants::LevelNamePointerTable +
                                                         passage * 24 + stage * 4);
    ...
    /// <param name="minutes">The number of minutes to set the timer for this difficulty
    /// level</param> <param name="seconds">The number of seconds to set the timer for this
    /// difficulty level</param>
    void Level::SetTimeCountdownCounter(enum __LevelDifficulty LevelDifficulty,
    ...
    void Level::RedistributeDoor()
    {
        ...
        // Check if every Room have at least one Door, if not, set the entityset id to skip some
        // problems the code is only for avoiding crash
    ...
    void Level::GetSaveChunks(QVector<ROMUtils::SaveData> &chunks)
    {
            ...
            }))
        {
            ...
            unsigned int cameraPointerTablePtr =
                WL4Constants::CameraControlPointerTable + LevelID * 4;
            int boundaryEntries = std::count_if(rooms.begin(), rooms.end(), [](Room *R) {
                return R->GetCameraControlRecords().size();
            });
            unsigned int cameraPointerTableSize = (boundaryEntries + 1) * 4;
            cameraPointerTable =
                (struct ROMUtils::SaveData *) malloc(sizeof(struct ROMUtils::SaveData));
            ...
            *(int *) (cameraPointerTable->data + boundaryEntries * 4) = GBAptrSentinel;
            ...
            // Create null entries in the chunk data which will be used to invalidate old camera
            // boundary chunks
            unsigned int cameraBoundaryListEntryPtr =
                ROMUtils::PointerFromData(cameraPointerTablePtr);
        ...
        for (unsigned int i = 0; i < rooms.size(); ++i)
        {
            ...
            memcpy(roomHeaders.data + i * sizeof(struct __RoomHeader), &rh,
                   sizeof(struct __RoomHeader));
            rooms[i]->GetSaveChunks(chunks, &roomHeaders, cameraPointerTable,
                                    &cameraPointerTableIndex);
        }
        ...
        for (unsigned int i = 0; i < doors.size(); ++i)
        {
            ...
            memcpy(doorChunk.data + i * sizeof(struct __DoorEntry), &entryStruct,
                   sizeof(struct __DoorEntry));
        }
        memset(doorChunk.data + doors.size() * sizeof(struct __DoorEntry), 0,
               sizeof(struct __DoorEntry));
        ...
        assert(LevelName.size() ==
               26 /* Level name must be internally represented as 26 characters in length */);
