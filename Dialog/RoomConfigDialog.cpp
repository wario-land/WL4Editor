...
RoomConfigDialog::RoomConfigDialog(QWidget *parent,
                                   DialogParams::RoomConfigParams *CurrentRoomParams) :
        QDialog(parent),
        ui(new Ui::RoomConfigDialog)
{
    ...
    ui->ComboBox_LayerPriority->setCurrentIndex((LayerPriorityID < 2) ? LayerPriorityID
                                                                      : (LayerPriorityID - 1));
    ui->ComboBox_AlphaBlendAttribute->setCurrentIndex(
        (CurrentRoomParams->LayerPriorityAndAlphaAttr & 0x78) >> 2);
    if (CurrentRoomParams->Layer0Enable)
    {
        ui->ComboBox_Layer0MappingType->setCurrentIndex(
            (((CurrentRoomParams->Layer0MappingTypeParam) & 0x30) >> 4) - 1);
    }
    ...
    std::vector<int> CurrentBGLayerdataPtrs =
        BGLayerdataPtrs[CurrentRoomParams->CurrentTilesetIndex];
    ...
    {
        ui->ComboBox_BGLayerPicker->addItem(
            QString::number(CurrentRoomParams->BackgroundLayerDataPtr, 16).toUpper());
    ...
    currentTileset =
        new LevelComponents::Tileset(_tilesetPtr, CurrentRoomParams->CurrentTilesetIndex);
    int L0ptr = (ui->ComboBox_Layer0MappingType->currentIndex() == 1)
                    ? CurrentRoomParams->Layer0DataPtr
                    : 0;
    ;
    ui->graphicsView->UpdateGraphicsItems(currentTileset, CurrentRoomParams->BackgroundLayerDataPtr,
                                          L0ptr);
...
DialogParams::RoomConfigParams RoomConfigDialog::GetConfigParams()
{
    ...
    if (configParams.Layer0Enable)
    {
        ...
        {
            configParams.Layer0MappingTypeParam =
                (ui->ComboBox_Layer0MappingType->currentIndex() + 1) << 4;
        }
        else
        {
    ...
    configParams.LayerPriorityAndAlphaAttr +=
        (ui->ComboBox_AlphaBlendAttribute->currentIndex() << 2);
    ...
    {
        configParams.BackgroundLayerDataPtr =
            ui->ComboBox_BGLayerPicker->currentText().toUInt(nullptr, 16);
    }
    else
    {
...
void RoomConfigDialog::StaticComboBoxesInitialization()
{
    ...
    for (unsigned int i = 0; i < sizeof(LayerPrioritySetData) / sizeof(LayerPrioritySetData[0]);
         ++i)
    { LayerPrioritySet << LayerPrioritySetData[i]; }

    // Initialize the selections for the alpha blending types
    for (unsigned int i = 0; i < sizeof(AlphaBlendAttrsSetData) / sizeof(AlphaBlendAttrsSetData[0]);
         ++i)
    { AlphaBlendAttrsSet << AlphaBlendAttrsSetData[i]; }

    // Initialize the selections for the types of layer mapping for layer 0
    for (unsigned int i = 0;
         i < sizeof(Layer0MappingTypeParamSetData) / sizeof(Layer0MappingTypeParamSetData[0]); ++i)
    { Layer0MappingTypeParamSet << Layer0MappingTypeParamSetData[i]; }

    // Initialize the selections for the tilesets's available BGs
    for (unsigned int i = 0, idx = 0;
         idx < sizeof(BGLayerdataPtrsData) / sizeof(BGLayerdataPtrsData[0]); ++i)
    {
        std::vector<int> vec;
        int count = BGLayerdataPtrsData[idx++];
        while (count--) { vec.push_back(BGLayerdataPtrsData[idx++]); }
...
void RoomConfigDialog::on_CheckBox_Layer0Alpha_stateChanged(int state)
{
    ...
    if (state == Qt::Unchecked) { ui->ComboBox_AlphaBlendAttribute->setCurrentIndex(0); }
}
...
void RoomConfigDialog::on_ComboBox_TilesetID_currentIndexChanged(int index)
{
    if (ComboBoxInitialized)
    {
        ...
        if (BGLayerdataPtrs[index].size() > 0) { ui->CheckBox_BGLayerEnable->setChecked(true); }
        else
        {
        ...
        ui->CheckBox_Layer0Enable->setChecked(
            true); // trigger on_CheckBox_Layer0Enable_stateChanged()
...
void RoomConfigDialog::on_CheckBox_BGLayerEnable_stateChanged(int state)
{
    ...
    ui->ComboBox_BGLayerPicker->setEnabled(state == Qt::Checked &&
                                           BGLayerdataPtrs[tilesetIndex].size());
    ...
    for (unsigned int i = 0; i < BGLayerdataPtrs[tilesetIndex].size(); ++i)
    {
        ...
        {
            ui->ComboBox_BGLayerPicker->addItem(
                QString::number(BGLayerdataPtrs[tilesetIndex][i], 16).toUpper());
        }
    }
}
