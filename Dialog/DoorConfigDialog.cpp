        ...
        DoorID(doorID)
{
    ...
    ui->TableView_EntityFilter->horizontalHeader()->setSectionResizeMode(
        QHeaderView::ResizeToContents);
    ui->TableView_EntityFilter->resizeColumnsToContents();
    // set rwo height
    ui->TableView_EntityFilter->verticalHeader()->setSectionResizeMode(
        QHeaderView::ResizeToContents);
    ...
    ui->ComboBox_DoorDestinationPicker->setCurrentIndex(
        currentdoor->GetDestinationDoor()->GetGlobalDoorID());
...
void DoorConfigDialog::EntitySetsInitialization()
{
    ...
    for (unsigned int i = 0; i < sizeof(entities) / sizeof(entities[0]); ++i)
    {
        ...
        entities[i] =
            new LevelComponents::Entity(tmpEntitysetAndEntitylocalId.entitylocalId, i,
...
void DoorConfigDialog::RenderGraphicsView_Preview()
{
    ...
    tparam.mode.hiddencoinsEnabled = tparam.mode.entitiesEnabled = tparam.mode.cameraAreasEnabled =
        false;
...
void DoorConfigDialog::ResetDoorRect()
{
    ...
    // when DestinationDoor and currentDoor are in the same Room, the DestinationDoor also needs an
    // update.
...
{
    EntityFilterTableModel *model =
        static_cast<EntityFilterTableModel *>(ui->TableView_EntityFilter->model());
    model->clear();
...
void DoorConfigDialog::on_TableView_Checkbox_stateChanged(QStandardItem *item)
{
    ...
    if (item->checkState() == Qt::Checked)
    {
        ...
        {
            if (set.visible &&
                !entitiessets[set.id]->IsEntityInside(it.entity->GetEntityGlobalID()))
            { set.visible = false; }
        }
        UpdateComboBoxEntitySet();
    }
    else if (item->checkState() == Qt::Unchecked)
    {
        ...
        for (auto &set : comboboxEntitySet)
        {
            ...
            for (int i = 0; i < model->rowCount(); ++i)
            {
                ...
                if (checked)
                {
                    if (!entitiessets[set.id]->IsEntityInside(
                            model->entities[i].entity->GetEntityGlobalID()))
                    {
                        visible = false;
                        break;
                    }
...
/// <summary>
/// Called when current index of ComboBox_DoorDestinationPicker is changed even on its
/// initialization time.
/// </summary>
/// <param name="index">
...
void DoorConfigDialog::on_ComboBox_DoorDestinationPicker_currentIndexChanged(int index)
{
    delete tmpDestinationRoom;
    tmpDestinationRoom = new LevelComponents::Room(
        _currentLevel->GetRooms()[_currentLevel->GetDoors()[index]->GetRoomID()]);
    tmpDestinationRoom->SetDoorsVector(
...
void DoorConfigDialog::on_ComboBox_DoorType_currentIndexChanged(int index)
{
    ...
    if ((index == 0) && (currentdoor0->GetDoorTypeNum() != 1))
    {
        QMessageBox::information(this, QString("Info"),
                                 QString("Unless you know what you are doing, don't put more than "
                                         "1 Portal-type Door (vortex) in one level."));
    }
...
/// The parent QWidget.
/// </param>
EntityFilterTableModel::EntityFilterTableModel(QWidget *_parent) :
        QStandardItemModel(_parent), parent(_parent)
{
    // nothing
}
