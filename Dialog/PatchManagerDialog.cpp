...
/// The parent QWidget.
/// </param>
PatchManagerDialog::PatchManagerDialog(QWidget *parent) : QDialog(parent), ui(new Ui::PatchManagerDialog)
{
    ui->setupUi(this);
...
/// </summary>
PatchManagerDialog::~PatchManagerDialog() { delete ui; }
...
retry:
    if (editDialog->exec() == QDialog::Accepted)
    {
        ...
        if (entry.ThumbMode && (entry.HookAddress & 1) || // Mis-aligned instruction check
            !entry.ThumbMode && (entry.HookAddress & 3)   // 2 bytes for thumb, 4 bytes for arm
        )
        {
            QMessageBox::information(
                this, "About", QString("Misaligned hook address: ") + QString("%1").arg(entry.HookAddress, 0, 16));
        }
        ...
        {
            QMessageBox::information(
                this, "About", QString("File must be within directory subtree of ROM file: ") + ROMUtils::ROMFilePath);
            goto error;
        }
        entry.FileName = relativeFN;
        bool fileNameIsValid =
            !std::any_of(currentEntries.begin(), currentEntries.end(),
        ...
        if (!fileNameIsValid) // Cannot add two patch entries using the same file
        {
            QMessageBox::information(this, "About",
                                     QString("Another entry already exists with the filename: ") + entry.FileName);
            goto error;
        }
        bool hookIsValid = !entry.HookAddress || !std::any_of(currentEntries.begin(), currentEntries.end(),
                                                              [entry](const struct PatchEntryItem &e) {
                                                                  return e.HookAddress == entry.HookAddress;
                                                              });
...
void PatchManagerDialog::on_editPatchButton_clicked()
{
    ...
    if (selectedRows.size() == 1)
    {
        ...
        std::find_if(currentEntries.begin(), currentEntries.end(),
                     [selectedEntry, &selectedIndex](const struct PatchEntryItem &e) {
                         ++selectedIndex;
                         return selectedEntry.FileName == e.FileName;
                     });
    ...
    retry:
        if (editDialog->exec() == QDialog::Accepted)
        {
            ...
            {
                QMessageBox::information(this, "About",
                                         QString("File must be within directory subtree of ROM file: ") +
                                             ROMUtils::ROMFilePath);
                goto error;
            }
            entry.FileName = relativeFN;
            bool fileNameIsValid =
...
void PatchManagerDialog::on_savePatchButton_clicked()
{
    ...
    while (!ok)
    {
        ...
        QString URL = "https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/"
                      "gnu-rm/downloads";
        ...
        QString selectedDir = QFileDialog::getExistingDirectory(this, tr("Select EABI installation"), invalidDir,
