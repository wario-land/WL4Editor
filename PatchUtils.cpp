...
#define REPLACE_EXT(qstr, fromstr, tostr) \
    do                                    \
    {                                     \
...
static QString GetUpgradedPatchListChunkData(unsigned int chunkDataAddr)
{
    ...
    QString contents =
        QString::fromLocal8Bit(reinterpret_cast<const char *>(ROMUtils::CurrentFile + chunkDataAddr + 13), contentSize);
...
static QString ExtractOFile(const QString &ofile)
{
    ...
    args << "-O"
         << "binary"
         << "--only-section=.text" << ofile << outfile;
...
static bool BinaryMatchWithROM(const QString &file, unsigned int startAddr, unsigned int length)
{
    ...
    if (startAddr + length > ROMUtils::CurrentFileSize)
        return false;
    QFile binFile(file);
...
namespace PatchUtils
{
    ...
    QVector<struct PatchEntryItem> GetPatchesFromROM()
    {
        ...
        unsigned int patchListAddr = ROMUtils::FindChunkInROM(ROMUtils::CurrentFile, ROMUtils::CurrentFileSize,
        ...
        if (patchListAddr)
        {
            ...
            assert(!(patchTuples.count() %
                     4) /* ROM contains a corrupted patch list chunk (field count is not a multiple of 4) */);
            for (int i = 0; i < patchTuples.count(); i += 4)
            {
                ...
                {
                    patchTuples[i], static_cast<enum PatchType>(patchType), hookAddress, stubFunction, thumbMode,
                        patchAddress, patchTuples[i + 6]
                };
    ...
    QString SavePatchesToROM(QVector<struct PatchEntryItem> entries)
    {
        ...
        for (int i = 0; i < entries.size(); ++i)
        {
            ...
            struct PatchEntryItem *existingPatch =
                std::find_if(existingPatches.begin(), existingPatches.end(),
                             [entry](const struct PatchEntryItem &e) { return e.FileName == entry.FileName; });
            bool mustCreateChunk;
            bool saveChunkExists = existingPatch != existingPatches.end();
            ...
            if (mustCreateChunk)
            {
                ...
                // Create the save chunk
                struct ROMUtils::SaveData patchChunk = { 0,
        ...
        bool ret           = ROMUtils::SaveFile(
            ROMUtils::ROMFilePath, chunks,
            // ChunkAllocationCallback
            [firstCallback, entries, chunkIndexToEntryIndex](QVector<struct ROMUtils::SaveData> addedChunks,
                                                             std::map<int, int> indexToChunkPtr) mutable {
                ...
                if (firstCallback)
                {
                    ...
                    foreach (struct ROMUtils::SaveData chunk, addedChunks)
                    {
                        ...
                        // TODO Capture data from hook address for the entry's substituted bytes (depends on size of
                        // hook)
                    }
                    addedChunks.clear();
