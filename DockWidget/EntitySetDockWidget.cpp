...
EntitySetDockWidget::EntitySetDockWidget(QWidget *parent) :
        QDockWidget(parent), ui(new Ui::EntitySetDockWidget)
{
    ui->setupUi(this);
...
void EntitySetDockWidget::ResetEntitySet(LevelComponents::Room *currentroom)
{
    ...
    ui->label_EntitySetID->setText("EntitySet ID: " +
                                   QString::number(currentroom->GetCurrentEntitySetID(), 10));
...
void EntitySetDockWidget::RenderEntityAndResetInfo()
{
    ...
    LevelComponents::Entity *currentEntityPtr =
        currentRoom->GetCurrentEntityListSource()[currentEntityId];
    ...
    LevelComponents::EntityPositionalOffset position =
        LevelComponents::EntitySet::GetEntityPositionalOffset(
            currentEntityPtr->GetEntityGlobalID());
    EntityBoxPainter.drawRect(-((position.XOffset + 98) / 4 + currentEntityPtr->GetXOffset() + 8),
                              -((position.YOffset + 66) / 4 + currentEntityPtr->GetYOffset() + 16),
                              16, 16);
    ...
    ui->textEdit_EntityInfo->append("Entity global Id: " +
                                    QString::number(currentEntityPtr->GetEntityGlobalID()));
}
